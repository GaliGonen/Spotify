...
def create_selective_mega_playlist_unique_artists():
    """Create a mega playlist from selected playlists with only one song per artist"""
    print("Creating Mega Playlist from selected playlists (one song per artist)...")
    ...
    try:
        ...
        # Step 3: Collect tracks from selected playlists (one per artist)
        print(f"\nCollecting tracks from {len(selected_playlists)} playlists (one per artist)...")
        all_tracks = {}  # Use dict to avoid duplicates by track ID
        artists_used = set()  # Track artists we've already included
        ...
        for i, playlist in enumerate(selected_playlists, 1):
            ...
            # Add tracks to our collection (avoiding duplicates by track AND artist)
            playlist_added = 0
            for item in tracks:
            ...
            print(f"      Added {playlist_added} unique artist tracks (Total unique artists: {len(all_tracks)})")
        
        if not all_tracks:
            print("No tracks found in selected playlists.")
            return None
        
        print(f"\nCollected {len(all_tracks)} unique tracks from {len(all_tracks)} unique artists")
        
        # Step 4: Create new mega playlist
        mega_playlist_name = f"Mega Playlist - Unique Artists ({len(all_tracks)} tracks)"
        ...
        try:
            mega_playlist = sp.user_playlist_create(
                ...
                description=f"Auto-generated mega playlist containing {len(all_tracks)} unique tracks from {len(all_tracks)} unique artists across {len(selected_playlists)} selected playlists. One song per artist only. Created on {time.strftime('%Y-%m-%d')}"
            )
        ...
        print(f"\nSUCCESS! Created mega playlist with {added_count} tracks from {len(all_tracks)} unique artists!")
        ...
        print(f"  • Found {len(all_tracks)} unique artists")
...
if __name__ == "__main__":
    playlist_id = create_selective_mega_playlist_unique_artists()
    if playlist_id:
        print("\nYour unique artists mega playlist is ready to enjoy!")
    else:
